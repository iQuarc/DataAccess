namespace iQuarc.DataAccess
{
        ...
        where T : class
    {
        ...
        public abstract void OnSave(IEntityEntry<T> entry, IRepository repository);
        public abstract void OnDelete(IEntityEntry<T> entry, IRepository repository);
        ...
        void IEntityInterceptor.OnSave(IEntityEntry entry, IRepository repository)
        {
            if (entry.Entity is T)
                this.OnSave(entry.Convert<T>(), repository);
        }

        void IEntityInterceptor.OnDelete(IEntityEntry entry, IRepository repository)
        {
            if (entry.Entity is T)
